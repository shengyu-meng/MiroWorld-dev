...
describe('app routes', () => {
  ...
  it('loads the stage route, allows branch selection, language switching, and archive drawer toggle', async () => {
    ...
    await flushPromises()
    for (let attempt = 0; attempt < 10 && createObjectURLMock.mock.calls.length < 11; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 10))
    }
    expect(canvasToDataURLMock).toHaveBeenCalledTimes(2)
    expect(createObjectURLMock).toHaveBeenCalledTimes(11)
    expect(anchorClickMock).toHaveBeenCalledTimes(10)
    expect(revokeObjectURLMock).toHaveBeenCalledTimes(11)
